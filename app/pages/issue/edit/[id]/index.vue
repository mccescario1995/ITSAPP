...
// Fetch existing issue data
const fetchIssue = async () => {
  if (!id.value) return;
  loadingIssue.value = true;
  try {
    const res: any = await api.getIssue(id.value);
    const issue = res.items?.result ?? null;

    console.log("Response: ", res);

    if (issue) {
      form.issueDetails = issue.issueDetails || "";
      form.actionPlan = issue.actionPlan || null;
      form.issueTypeId = issue.issueTypeId;
      form.responsibleGroupId = issue.responsibleGroupId;
      form.responsibleEmpId = issue.responsibleEmployeeId || null;
      form.status = issue.status;

      if (issue.responsibleEmployeeId && issue.responsibleEmployee) {
        selectedEmployee.value = {
          value: issue.responsibleEmployeeId,
          label: issue.responsibleEmployee,
        };
      }
...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
  ...
  try {
    ...
    const response = await api.updateIssue(id.value, data);
...
<template>
  <div>
    ...
    <div v-else class="space-y-4">
      <h1 class="text-2xl font-semibold mb-4">Edit Issue #{{ id }}</h1>
      <UForm
        :schema="schema"
        :state="form"
        @submit="onSubmit"
      ...
      >
        ...
        <UCard class="rounded-xl shadow-sm">
          ...
          <UFormField label="Assigned to:" name="responsibleEmpId" class="mt-4">
            <USelectMenu
